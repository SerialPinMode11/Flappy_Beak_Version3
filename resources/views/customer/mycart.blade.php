...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div class="mb-8">
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-gold-deep/90 mb-2">Checkout</p>
            <h2 class="font-serif text-2xl sm:text-3xl font-semibold text-forest">Your shopping cart</h2>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('cart.update-quantity') }}" method="POST" class="flex items-center gap-2">
                                        ...
                                        <input type="number" name="quantity" value="{{ $details['quantity'] }}" min="1" class="w-16 px-2 py-1 border border-stone-200 rounded-lg focus:ring-2 focus:ring-gold/40 focus:border-forest" onchange="this.form.submit()">
                                        <button type="submit" class="text-xs px-2 py-1 rounded-lg bg-cream hover:bg-stone-100 text-stone-700 border border-stone-200">Update</button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
