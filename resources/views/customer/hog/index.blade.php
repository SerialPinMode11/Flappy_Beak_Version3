...
@section('title', 'Hog products — ' . ($publicContent['store_name'] ?? 'JM Casabar Mini Farm'))

@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        <div class="mb-10">
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-gold-deep/90 mb-2">Prime cut</p>
            <h1 class="font-serif text-2xl sm:text-3xl font-semibold text-forest">Hog products</h1>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-2 md:gap-3 mb-10">
            ...
            <a href="{{ route('wine.home') }}" class="px-5 py-2.5 bg-forest text-white rounded-full text-sm font-medium hover:bg-forest-dark transition-colors flex items-center gap-2 shadow-sm">
            ...
            <a href="{{ route('hog.home') }}" class="px-5 py-2.5 bg-stone-800 text-white rounded-full text-sm font-medium hover:bg-stone-900 transition-colors flex items-center gap-2 ring-2 ring-gold/25">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8 mb-12">
            @foreach($hog_products as $product)
                <div class="product-card bg-white rounded-2xl p-4 relative group border border-stone-200/80 shadow-md hover:border-gold/35 transition-colors">
                    <span class="absolute top-4 left-4 bg-stone-800 text-gold-pale text-xs px-3 py-1 rounded-full font-semibold z-10">Featured</span>
