        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="p-8">
                <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <!-- Category (for home page filter: Duck / Egg) -->
                    <div class="mb-6">
                        <label for="category" class="block text-sm font-medium text-gray-700 mb-2">Category</label>
                        <select id="category" name="category" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary">
                            <option value="duck" {{ old('category', $product->category ?? 'duck') === 'duck' ? 'selected' : '' }}>Duck</option>
                            <option value="egg" {{ old('category', $product->category ?? '') === 'egg' ? 'selected' : '' }}>Egg</option>
                        </select>
                    ...
                    <div class="flex justify-end space-x-4">
                        ...
                        <button type="submit" 
                            class="px-6 py-3 bg-secondary text-gray-700 rounded-lg hover:bg-opacity-90 transition-colors">
                            Update Product
                        </button>
                    </div>
                </form>
